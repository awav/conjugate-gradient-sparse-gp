...
class CoverTreeNode:
    def __init__(self, point, radius, parent, data, r_neighbors: Optional[list] = None):
        self.point = point
        self.radius = radius
        self.parent = parent
        self.data = data
...
class CoverTree:
    def __init__(
        ...
        warnings.warn(
            "Distance function will be ignored and instead `numpy.linalg.norm` will be used."
        )
        ...
        if voronoi:
            root.voronoi_data = (root.data[0].copy(), root.data[1].copy())
        if plotting:
            root.plotting_data = (root.data[0].copy(), root.data[1].copy())
            ...
            for parent in self.levels[level - 1]:
                ...
                for child in parent.children:
                    child.r_neighbors = [
                        ...
                        if self.distance((r_neighbor.point, child.point)) <= neighbor_factor[level] * radius
                    ]
                    if plotting:
