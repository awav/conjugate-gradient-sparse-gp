...
def main(output, files: Sequence[Union[Path, str]]):
    """
    ...
    def flat_clustering_args(struct):
        args = struct["clustering_args"]
        new_struct = copy.deepcopy(struct)
        key = "clustering_args"
        new_struct = new_struct | {f"{key}_{k}": v for k, v in struct[key].items()}
        del new_struct[key]
        return new_struct

    results_flat = [flat_clustering_args(struct) for struct in results]
    df = pl.DataFrame(results_flat)
